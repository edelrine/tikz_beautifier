...
def beautifier(file, multidimensional=False, **options):
    ...
    set multidimensional=True if you want to get the multidensional array
    and not the formatted string
    '''
    ...
    run(sort_lines,
        not options.get('no_sort', False), latex, options
        )
    ...
    def get_result(latex, options):
        strip = options.get('no_strip', False) is False
...
def beautifier_CLI(path_file, **options):
    ...
    def save(path_file):
        ...
        file_to_save = ''.join(
            [p + '/' for p in path_file.split('/')[:-1]]) + name + '_clear.tikz'
...
if __name__ == '__main__':
    ...
    sorting.add_argument(
        ...
        action='store_true')
    sorting.add_argument(
        '-decreasing-abscissa', '-da',
        help='sorted abscissa in decreasing order',
        action='store_true')
    sorting.add_argument(
        '-decreasing-ordinate', '-do',
